...
class OrganizationalStructureController extends BaseController
{
    ...
    public function index()
    {
        ...
        $result = $organizationalStructure->field("id,pid,department_name as label,id as value")->order("id asc")->select();
        $list = $organizationalStructure->table("oa_organizational_structure as a")
            ->join("oa_basic_information as b on b.department_id=a.id")
            ->field("a.department_name,a.pid,b.department_id,count(1) as num")->group("b.department_id")->select();
        foreach($result as $key=>$value){
            foreach($list as $k=>$v){
                if($v["department_id"]==$value["id"]){
                    if($v["pid"] == '0'){
                        $result[$key]["counts"] = count($k);
                    }else{
                        $result[$key]["counts"] = $v["num"];
                    }
                }
            }
        }
    ...
    public function add()
    {
        ...
        if($deptNum['department_number']){
            self::jsons($error=0, $result="部门编码已经存在，请重新设置部门编码");
    ...
    public function delete()
    {
        ...
        $organizationalStructure = D("OrganizationalStructure");
        $id = array($id);
        $owhere = 'pid in(' . implode(',', $id) . ')';
        $existing = $organizationalStructure->field("pid")->where($owhere)->select();
        $basicInformation = D("BasicInformation");
        $bwhere = 'department_id in(' . implode(',', $id) . ')';
        $basExisting = $basicInformation->field("department_id")->where($bwhere)->select();
        if($existing){
            self::jsons($error = 0,$result="存在子部门,不能删除");
        }
        if($basExisting){
            self::jsons($error = 0,$result="部门下存在员工,不能删除");
        } else{
            if ($length == 1) {
                $where = 'id=' . $id;
            } else {
//                $id = array($id);
    ...
    public function export(){
        ...
        $result = $organizationalStructure->table("oa_organizational_structure as a")->join("left join oa_organizational_structure as b on a.pid=b.id")
            ->field("a.id,b.department_name as pname,a.department_name,a.department_number,a.department_heads,date_format(from_unixtime(a.setup_time),'%Y-%m-%d') as setup_time,a.url")
            ->order("a.department_number asc")
            ->select();
        if($result){
            $field = array(
